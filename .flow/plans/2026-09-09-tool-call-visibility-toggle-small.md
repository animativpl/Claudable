# Tool Call Visibility Toggle (small lane)

> **For agentic workers:** REQUIRED SUB-SKILL: Use subagent-driven-development to implement this task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Decision:** Add a client-side toggle in `ChatLog.tsx`, default **off**
(tool-call messages hidden), that shows/hides `tool_use`/`tool_result`
messages (and anything `isToolUsageMessage` matches) in the rendered chat
log. State is held in `sessionStorage` via a new
`lib/utils/tool-visibility-storage.ts` helper, mirroring the existing
`lib/utils/model-selection-storage.ts` pattern exactly (same read/write
shape, same "read inside a `useEffect` after mount, not in the `useState`
initializer" convention already used in `app/page.tsx:60-84` to avoid a
Next.js hydration mismatch between server and client first paint).
Approved by the user 2026-09-09.
