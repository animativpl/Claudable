...
| 15 | ESLint config Next 16 — nowy ruleset `typescript-eslint` | **Override `@typescript-eslint/no-explicit-any: 'warn'`** w nowym `eslint.config.mjs`, nie masowa naprawa kodu | Znalezisko drugiego red-teamu, zweryfikowane grepem: `eslint-config-next@16.3.3` po raz pierwszy włącza `typescript-eslint` recommended (poprzednio zero reguł TS), które stawia `no-explicit-any` na `error`. Repo ma dziś **49** wystąpień `any` (zmierzone grepem). Naprawa 49 miejsc to osobna robota, nie efekt uboczny bumpa zależności — override zachowuje dzisiejszy status quo (widoczność bez czerwonej bramki), tak samo jak decyzja 11 dla Turbopacka. |
