...
| 12 | Agent SDK — nowe peer dependencies | **Dodać `@anthropic-ai/sdk` i `@modelcontextprotocol/sdk` jawnie do `dependencies`**, rozszerzyć `Dockerfile:101` o kopiowanie `@modelcontextprotocol` | Znalezisko red-teamu, zweryfikowane wprost o rejestr npm i `Dockerfile`: 0.3.246 ma `peerDependencies` na oba pakiety (bez `peerDependenciesMeta`, czyli nieoptcjonalne), a `Dockerfile` kopiuje do runtime image wyłącznie scope `@anthropic-ai`. Bez poprawki: kontener startuje zdrowo, pierwsza wiadomość do agenta pada na `Cannot find module '@modelcontextprotocol/sdk'` — dokładnie ten sam typ awarii, przed którym broni istniejący komentarz przy linii 101. |
