...
| 12 | Chat history pagination | Cursor-based (`createdAt` timestamp), not offset-based. `GET /api/chat/[project_id]/messages` gains `order` (`asc`/`desc`, default `asc`) and `before` (ISO timestamp) params; `getMessagesByProjectId` takes `{ order?, before? }` instead of a numeric offset. Client fetches newest-first (`order=desc`) for the initial/polling load and pages backward via `before=<oldest loaded message's createdAt>` for "load older messages". | Offset-based pagination was broken two ways: the initial fetch used `offset=0` against an ascending sort, always returning the *oldest* messages instead of the most recent conversation; and "load older" derived its offset from the client's deduplicated/expanded message array length rather than the real raw row count, causing each click to re-fetch already-seen rows that then got discarded by ID dedup (~1 new message per click instead of a batch). Cursor pagination also avoids offset drift as new messages arrive between polls. Design record: `.flow/specs/2026-09-09-chat-history-pagination-design.md`. |
