...
| 11 | Agent session lifecycle | One persistent per-project `Query` (Agent SDK), created via an open `AsyncIterable` prompt fed with `query.streamInput()` for every chat message, instead of one fresh subprocess per message via `resume: sessionId`. `perTaskStopAffordance: true`. A new `agent-session-registry.ts` (mirrors `PreviewManager`) lets `instrumentation-node.ts`'s `shutdown()` kill it synchronously, same as preview processes. `reconcileStaleRequests()` attempts auto-resume via the saved `activeClaudeSessionId` before falling back to `failed`. **Decided 2026-09-02, not yet implemented — see §3 for current (pre-change) architecture.** | Confirmed on the deployed instance that `executeClaude()`'s one-shot string `prompt` closes stdin, which per the Agent SDK's own docs (`perTaskStopAffordance`) makes the CLI kill any background subagent/task still running when a turn's result is released — every turn, independent of server/container restarts. A second, separate gap (the agent subprocess was never registered for `shutdown()`'s synchronous kill, unlike preview processes) meant a container restart also orphaned it uncleanly. Full deliberation, evidence, and rejected approaches: `.flow/specs/2026-09-02-persistent-agent-session-design.md`. |
